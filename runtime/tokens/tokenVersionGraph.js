...
function hasVersion(entries, versionId) {
    return typeof versionId === 'string' && versionId.length > 0 && Boolean(entries?.[versionId]);
}
...
export function appendTokenVersion(graphInput, entryInput) {
    ...
    const nextOrder = [...order, versionId];

    return buildNextGraph(graph, nextEntries, nextOrder, versionId);
}
...
export function rollbackTokenVersion(graphInput, rollbackTargetId) {
    ...
    if (!hasVersion(entries, rollbackTargetId)) {
        return graph;
    }

    return buildNextGraph(graph, entries, Array.isArray(graph.order) ? graph.order : [], rollbackTargetId);
}
