...
export default function Canvas() {
  // 🔑 Dispatcher comes ONLY from workspace context
  ...
  // ─────────────────────────────────────────────
  // DEV DEBUG API (NO ID GENERATION, NO BYPASS)
  // ─────────────────────────────────────────────
  useEffect(() => {
    ...
    return () => {
      if (window.__droppleDebug === api) {
        prev ? (window.__droppleDebug = prev) : delete window.__droppleDebug;
      }
    };
  }, [dispatcher, clear, selectSingle, setSelection]);
