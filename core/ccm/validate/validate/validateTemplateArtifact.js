...
function detectCycleInTree(tree, root) {
...
export function validateTemplateArtifact(artifact) {
    ...
    for (const key of keys) {
        if (!TEMPLATE_V1_KEYS.includes(key)) {
    ...
    for (const field of REQUIRED_METADATA_FIELDS) {
        const value = typed.metadata[field];
    ...
    if (detectCycleInTree(structure.tree, structure.root)) {
        throw new ValidationError("structure.tree contains a cycle (cycles are not allowed)");
    }
    // STEP 4 — Motion
    const { motion } = typed;
    ...
    for (const name in motion.timelines) {
        ...
        for (const track of timeline.tracks) {
            if (!nodeIds.has(track.target)) {
                throw new ValidationError(`Motion track targets unknown node: ${track.target}`);
            }
            if (!ALLOWED_MOTION_PROPERTIES.includes(track.property)) {
    ...
    for (const group of ["content", "style", "motion"]) {
        const block = typed.params[group];
